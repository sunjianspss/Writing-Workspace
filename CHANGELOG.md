...
- When that cleanup itself fails, the error now names the version left behind instead of swallowing it. Previously the author saw only the projection error while an invisible pending version stayed in the database.
...
- Evaluation can re-score articles it has already generated, comparing two versions of the scoring prompt against the same text. Because both versions read identical drafts, the comparison is paired: the report states the average difference with a confidence interval, so a change that only looks like an improvement is reported as undecidable rather than as a win. The re-scoring run also records each problem the reviewer raised — its dimension, severity, quoted passage, and reasoning — which ordinary evaluation runs discard, keeping only counts.
