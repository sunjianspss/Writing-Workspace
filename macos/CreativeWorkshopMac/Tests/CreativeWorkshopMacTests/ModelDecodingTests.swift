...
final class ModelDecodingTests: XCTestCase {
    ...
    /// 回归（24.11 / P2-7）：大纲此前是 1200 字 prefix-only 掐尾，而大纲最后一段恰恰是"结尾设计"——
    /// 静默丢掉的正是收束方式。现在与素材同一种做法：留头留尾、写明省略了多少字。
    func testWritingContextBuilderKeepsTailOfLongOutline() {
        let head = String(repeating: "头", count: 1_000)
        let tail = String(repeating: "尾", count: 1_000)
        let context = WritingContextBuilder().build(
            title: "标题",
            summary: "",
            content: "",
        ...
        XCTAssertTrue(context.outline_excerpt.hasPrefix("头"), "大纲开头应保留")
        XCTAssertTrue(context.outline_excerpt.hasSuffix("尾"), "大纲结尾（结尾设计）不能再被掐掉")
        XCTAssertTrue(context.outline_excerpt.contains("（此处省略中间 800 字大纲）"), "省略处必须写明丢了多少字")
    }
