...
package enum NativePrompts {
    ...
    package static func argumentCheck(
        ...
        请只输出 JSON，不要输出 Markdown 代码块：
        {
          ...
          "weak_points": ["薄弱处"],
          "missing_evidence": ["缺少的证据/场景/个人经验"],
    ...
    package static func writingReview(
        ...
        6. 不要直接重写全文
        7. 如果存在"上一次诊断"，先判断其中的问题这次是否已经解决：已解决的问题写入 resolved_from_last，
           不要在 issues 里重复列出；issues 只列仍未解决的问题和新出现的问题
        8. 如果作者风格说明中列出了"作者常见雷区"，重点检查这些问题是否再次出现
    ...
    private static func agentDiscipline(mode: AgentPromptMode) -> String {
        ...
        case .argumentAudit:
            modeFocus = "论证审计：找出主张、证据、场景和个人经验之间的缺口，宁可指出不足，不要假装充分。"
        case .briefRevision:
