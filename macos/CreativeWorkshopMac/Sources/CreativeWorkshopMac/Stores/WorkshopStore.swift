...
final class WorkshopStore: ObservableObject {
    ...
    /// 归纳该风格档案下反复出现的作者雷区候选（18.3.3）：只生成候选，采纳与否由作者决定。
    func summarizeAuthorPitfalls() async {
        ...
        await run("归纳作者雷区", cancellable: true) {
            ...
            let response = await self.executeWorkflow(
                NativeWorkflowCatalog.summarizePitfalls(
                    issues: issues,
                    style: style,
                    template: self.promptTemplate(for: .pitfallSummary)
                )
            ).pitfallSummaryResponse
            try Task.checkCancellation()
            try self.recordAgentRun(
                runType: "归纳作者雷区",
                status: response.success == true ? "success" : "fallback",
                summary: response.result.candidates?.prefix(3).map(\.description).joined(separator: "；"),
            ...
            let existingDescriptions = Set((style.known_pitfalls ?? []).map(\.description))
            self.pitfallCandidates = (response.result.candidates ?? [])
                .filter { !existingDescriptions.contains($0.description) }
    ...
    func confirmPitfallCandidate(_ candidate: PitfallCandidate) async {
        ...
        await run("确认作者雷区") {
            var updated = style
            let newPitfall = AuthorPitfall(
                description: candidate.description,
                source_review_id: candidate.supporting_review_ids?.first,
                created_at: nil
            )
            var pitfalls = updated.known_pitfalls ?? []
            guard !pitfalls.contains(where: { $0.description == newPitfall.description }) else {
            ...
            let saved = try self.database.saveStyleProfile(id: style.id, profile: updated)
            self.styleProfiles = try self.database.listStyleProfiles()
            if self.selectedStyleProfileID == style.id {
                self.editStyleProfile(saved)
            }
            self.pitfallCandidates.removeAll { $0.description == candidate.description }
    ...
    /// 20.4：从发布前编辑记录归纳候选编辑偏好。候选只展示，必须人工确认后才注入 prompt。
    func summarizeEditPreferences() async {
        ...
        await run("归纳编辑偏好", cancellable: true) {
            let records = try self.database.listEditRecords(limit: 80)
            ...
            let response = await self.executeWorkflow(
                NativeWorkflowCatalog.summarizeEditPreferences(
                    records: records,
                    style: style,
                    template: self.promptTemplate(for: .editPreferenceSummary)
                )
            ).editPreferenceSummaryResponse
            try Task.checkCancellation()
            try self.recordAgentRun(
                runType: "归纳编辑偏好",
                status: response.success == true ? "success" : "fallback",
                summary: response.result.monthly_summary ?? response.result.candidates?.prefix(3).map(\.description).joined(separator: "；"),
                elapsedMS: response.elapsed_ms ?? 0,
                inputSummary: response.input_summary ?? "",
                outputSummary: response.output_summary ?? "",
                error: response.error ?? "",
            ...
            let existingDescriptions = Set((style.learned_preferences ?? []).map(\.description))
            self.editPreferenceCandidates = (response.result.candidates ?? [])
                .filter { !existingDescriptions.contains($0.description) }
            self.editRecordMonthlySummary = response.result.monthly_summary ?? (try? PublishingMetricsRecorder(database: self.database).snapshot().monthlySummary) ?? self.editRecordMonthlySummary
    ...
    func confirmEditPreferenceCandidate(_ candidate: EditPreferenceCandidate) async {
        ...
        await run("确认编辑偏好") {
            var updated = style
            let newPreference = LearnedPreference(
                description: candidate.description,
                source_edit_record_ids: candidate.source_edit_record_ids,
                created_at: nil
            )
            var preferences = updated.learned_preferences ?? []
            guard !preferences.contains(where: { $0.description == newPreference.description }) else {
    ...
    func resolveStyle() throws -> StyleProfile {
        ...
        let genreArticles = try database.recentArticlesForSamples(
            genre: genreKey,
            excludingArticleID: selectedArticleID,
            limit: 3
        )
        let genreExcerpts = genreArticles.compactMap { $0.content.nilIfEmpty }
