...
版本：原生化重构版  
日期：2026-07-01  
状态：产品演进与实施记录；当前产品意图以 `PRODUCT.md` 为准
主线形态：macOS 原生应用  
...
状态：已实施（评测驱动的条件项除外；2026-07-07 回填）。本章 2026-07-05 作为提案提出，经作者审查后拆解为任务 11–19（见 `创作工坊-23章改进-执行提示词.md`）：任务 11–16 已完成并经核验（全量测试通过、`architecture_guard.sh` 通过）；任务 17（write_section）为证据门条件任务，**终判不实施**（2026-07-08，截断/套话证据为零）；任务 18（调度偏好）**终判不实施**（2026-09-12，样本数达标但无成功会话可归纳）；任务 19 即本次回填。各节末尾附实施记录，正文保留提案时点的原始表述。

本章核心的 L2 有界代理循环已于 2026-09-12 **结案：放行门终判不转默认，开关保留、停止投入**（见 23.6.6 第三轮记录）。因此本章状态为"已实施并收束"，而非"进行中"。
...
**放行门第二轮记录**（2026-07-08，`evals/reports/2026-07-08T01-21-19Z.md`）：本轮 case-16 起约 40% 数据因评测中途合盖移机、唤醒时网络未就绪而被瞬时离线错误污染（报告表内的 18% 熔断率为事故产物）。以干净子集（case-01–15，剔除网络失败项）重算：deep 87.4 / direct 85.9 / agent 85.2 / agentic 77.0（含一次空交付 0 分；剔除后 82.9）——锚点评分已把分布从首轮的"72 扎堆"拉开到 75–95。**判定：放行门仍不通过（分数项差 4.5–10.4），L2 继续留在实验室**。会话轨迹带来两个新发现：agentic 已会提前 finish（预算用量 3–12 次不再恒满），但无人可问的评测环境里约半数会话以 ask_author 收尾，其中一例只生成大纲便提问、交付空正文得 0 分——评测的无头模式应移除 ask_author 动作，此为下一轮前的修缮项。
**第二轮后修缮（2026-07-08，已实施）**：① 无头评测移除 ask_author（`WritingAgentSessionInput.allowAskAuthor = false`）；② finish 前置收紧为"正文非空"（"不空手"，取代 23.6.2 的"恒可用"，App 内同样生效）；③ 评测韧性——瞬时离线失败自动丢弃重试、持续离线中止（`OfflineBreaker`）、结果增量落盘、新增 `--cases` 过滤参数。

**污染区补测与阶段收官（2026-07-08）**：用 `--cases` 分两次补齐 case-16～24（报告 `2026-07-08T15-42-40Z.md`、`2026-07-08T13-32-12Z.md`），全库 24 用例首次全部拥有真实数据。跨运行合并口径（同一锚点评分尺）：deep ≈ 86.5 > agent ≈ 85.7（仅 6 次调用，性价比最高）> direct ≈ 84.5 > agentic（修缮后）≈ 80.6。**放行门分数项维持不通过，L2 留实验室**；修缮全部经真实模型验证：agentic 无 ask_author 空转、finish 均交付非空正文、search_materials 首次被真实使用（case-24 两次检索，超时熔断后仍交付 91 分）、一次跨休眠的运行被增量落盘和熔断语义完整保住。评测线就此告一段落，重启条件：管线/prompt 实质改动、新用例批量入库或重议 L2 转默认。

**放行门第三轮与终判（2026-09-12，结案）**：最近一轮全量（`run_id` 8DF28A5D，112 格，12 格失败）三项判定——

- rubric 均分：agentic 84.44 vs deep 85.17，**差 0.73**。这一项的正确读法是**判不出，不是不通过**：差值标准误 1.40，0.73 只有 0.52 个标准误，95% CI −2.02~+3.48 跨 0。三轮下来 agentic 从"明确落后"（72.33 vs 75.00 → 77.0 vs 87.4 → 80.6 vs 86.5）走到了"分不出高下"，并已反超 agent（82.69）。
- 平均调用次数：agentic 11.76 vs deep 8.83 ≈ 1.33×，**通过**（阈值 1.5×）。
- 熔断率：四条管线 `fallback_count` 全为 0，**通过**。

**终判：不转默认，开关保留，停止投入。** 依据不是"它坏了"，而是三条：

1. 门槛写的是"agentic ≥ deep"，而现在的证据只到"判不出"。按 1.40 的标准误，要真判出 0.73 分的差距需要每管线约 174 例（现为 28 例）；按 112 格 8 小时折算，174×4 格要跑约 50 小时 API 时间。为一个默认关闭的入口付这个价钱不划算。
2. 即使补足样本判成"平"，也仍该维持不转默认：1.33 倍调用换来的是"一样好"而不是"更好"，而 deep 是确定性流程、agentic 是不确定的决策循环——同等质量下选确定的那个，这一步不需要更多数据。
3. 真实使用给出的是同向证据。App 内 9 次真实会话（`session_kind = 'agent_session'`）**无一以 `.finished` 收尾**：需要作者补充信息 2 次、模型自认改不动而交还作者 2 次、空转熔断 1 次、作者手动结束 1 次，其余 3 次跑到预算边上（12 次预算实际用掉 7～11 次）。它不是"做完了"，是"跑没了"。最后一次使用为 2026-07-30。

护栏本身是有效且完整的：七种停止条件（`finished` / `askAuthor` / `budgetExhausted` / `circuitBreak` / `invalidDecision` / `repeatedActionLimit` / `stalled`）均被真实会话触发过，UI 也已齐备（设置页开关、会话入口、提问卡、待复核卡会话摘要）。**结案的是"是否转默认"这个问题，不是这段代码的质量。**

落到工具上：agentic 退出默认评测管线（`EvalPipelineFacade.defaultPipelineNames`），每轮省掉四分之一；它仍是合法的 `--pipelines` 取值，复议时显式带上重跑即可，放行门照常判定。

**重启条件**（较此前收紧）：不再因"prompt 实质改动"自动重启——那只会让这个悬案每改一次量具就复活一次。只有出现下列之一才重开：① 作者在真实写作里重新使用 L2 并出现 `.finished` 收尾的会话；② deep 本身出现质量天花板，且证据指向"调度"而非"被调用的动作"。
...
2. `write_section`（任务 17，条件任务）：**终判不实施**（2026-07-08 复判）——第二轮评测的验证信号列显示：干净子集全部约 57 个真实产物中，疑似截断 0 例、结尾套话命中 0 例；"整篇一次生成"不是失分点，失分集中在 agentic 的调度决策质量（见 23.6.6 第二轮记录）。证据门两停一终判，本项复杂度确认省下；若未来评测出现截断/结构类信号占主导，可重新开题。
3. 调度偏好记忆（任务 18）：**终判不实施**（2026-09-12 复判）——样本数的解锁线已经过了（真实代理会话 9 条 ≥ 5 条），但**样本性质不对**：9 条里只有 5 条真正跑起来（≥7 步），且无一以 `.finished` 收尾。本项要归纳的是"哪类动作序列对这位作者收益高"，而这批数据里没有一条"收益高"的序列可归纳——能归纳出来的只有"作者总在中途接管"，那不是调度偏好，是失败模式；把它注入 `agentDecision` prompt 只会把失败模式固化成先验。

   解锁条件因此从"样本数 ≥5"改写为"存在以 `.finished` 收尾的真实会话 ≥5 条"。在 L2 已终判不转默认（见 23.6.6 第三轮记录）的前提下，这个条件短期内不会满足，本项随之关闭。
...
状态：滚动章节。系统自 2026-07 中旬进入真实使用阶段（作者已通过 App 完成鸳鸯、哀牢山等多篇文章），本章收录由真实使用暴露的改进项，逐条附实施记录。

### 24.1 发布流程引导（已实施，2026-07-12）

**背景**：真实使用数据显示作者连续多篇以"已归档"收尾、`edit_records` 为 0——北极星指标（发表编辑量/零改动率）只在状态翻"已发布"时记录，作者跳过发布站导致数据链断裂。根因是界面把"草稿/已发布/已归档"呈现为平级选项，流程的方向性不可见。

**实施**（护栏为主、可视化为辅）：
1. **归档护栏**：已有文章未经"已发布"直接翻"已归档"时弹确认——"先发布再归档"（一键补走完整链：发表前终审 → 编辑量记录 → 发布 → 归档）/"仍然归档"（弃稿语义保持诚实）/"取消"。入口收敛为 `requestArticleStatusChange`，UI 状态按钮一律经此。
2. **流程指示条**：状态控件旁渲染"构思 → 初稿 → 待复核 → 已发布 → 已归档"，当前站高亮，"已发布"站标注"发布时记录终审与编辑量"，让必经站自解释。
...
**验证结果（2026-07-25，作者本机实测）**：迁移先要落地——作者机器上 `prompt_templates.writing_review` 一直是 7-01 的 590 字符旧行（无锚点、无 `{{previous_review}}`），说明此前从未启动过含本修复的构建，验证条件根本不成立。重新构建安装并启动后该行刷新为 1269 字符，锚点/`{{previous_review}}`/`resolved_from_last` 三项俱全，迁移分支按设计生效（其余 15 个 key 内容未变，长度不动）。

随后对 id=15《叹香菱》连诊断三次（`writing_reviews` id 59/60/61，`reviewed_snapshot` 三条完全相同，2208 字）：

| # | 分数 | 问题清单 |
| --- | --- | --- |
| 59 | 83 | 人称视角(中)、意象与细节(中) |
| 60 | 73 | 人称视角(中)、意象与细节(中)、留白与节奏(低) |
| 61 | 72 | 人称视角(中)、意象与细节(中)、留白与节奏(中) |

**极差 11 分，未达 ≤5 的目标，但病灶已经换了**：三次识别出的问题几乎逐条重合（"我们"的导游口吻、"香菱跪着"解读概念化、结尾抒情），且分数严格跟着清单走——2 条中危 → 83（对应 80–89 档"1–2 处中低危"），3 条 → 72/73（对应 70–79 档"3 处以上中危"）。**同一份清单下的分数抖动只有 1 分**（73 vs 72），这才是量具噪声的真实水平，已达标。11 分极差来自第三条问题（结尾抒情）在首次被漏检，而它恰好跨在 79/80 的档界上，锚点把"漏检一条低危"放大成 10 分。

**裁决**：接受现状，**趋势改用问题清单而非分数来读**（issues 条数与严重度比分数可靠；分数在档界跳 10 分是锚点的设计后果，不是失真）。两条备选均不实施：给六个文学向维度加"逐一显式判定"以治漏检——留待漏检再次出现时再做；把档界从"问题条数"改为严重度加权——动锚点意味着分数再次不可比，收益不抵代价。

**第二例（同日，id=65 → 66，85 → 75）**：作者只改了三处引文（缘何/何缘、第七十九回→第八十回、劈头劈面→劈头劈脸浑身、末尾补句号），两版正文相似度 99.78%，分数掉 10 分。差异在于同一个问题被重新定级——"把基于判词的推测当作确凿事实"上一次记为「语言腔调·中」，这一次记为「文本引用与推断的边界·高」，而锚点规定有高危问题不得进 80 档；另有一条「废物也」引申在上一次被漏检（该句两版都在）。**机制与三连诊断完全一致：问题稳定，分数不稳定。** 作者复议后仍裁决不实施"逐一显式判定"——它只减少"这次才发现"类波动，不解决严重度定级本身的摇摆，而后者才是 10 分跳变的主因。

**已知边界与待观察**：
- ~~`previousReviewBlock` 的锚定/对抗性偏移~~ **已由三连诊断数据否定**：若存在锚定，第 60 条应贴近上一轮的 83，实际掉到 73；第 61 条摘要明写"上一轮诊断指出的问题均未解决"。喂历史带来的是趋严，不是趋同，无需改动。
- 内联 prompt 与 seed 模板仍是两份文案（仅锚点已共用）。彻底消除重复需让 seed 由内联 prompt 派生，属结构性改动，本次未做。**该断链在评测侧的另一半已由 24.5 收口**（评测改走库内模板），但代码里两份文案并存的事实不变。
...
**未处理**：`outline_excerpt` 仍是 1200 字 prefix-only 截断（同类缺陷，但无证据表明当前用量下会触发，暂不动）。~~`resolveStyle` 的兜底分支只有数据库层测试覆盖，未加 WorkshopStore 级行为测试。~~ 已由 24.6 补上（`testResolveStyleSkipsDraftsAndCrossFamilyArticlesWhenPickingSamples`）。
...
### 24.4 评测注入固定风格样本（已实施，2026-07-25）

**背景**：24.3 记录了「评测跳过样本注入、量的是作者用不到的零样本配置」，但当时判定不可直接对齐——评测用例按「一鱼两吃」惯例取自作者真实文章，naive 注入会把答案当样本喂回模型。本节给出裁决后的实施方案。

**两条决定性证据**：
1. **自动识别同源不可靠**。`articles.id=15`（香菱）确是 case-27 正文，但 `id=11`（鸳鸯）与 case-25 的前缀指纹**匹配失败**——文章正文带 YAML frontmatter（`---\ntitle: 鸳鸯...`），用例是 `# 鸳鸯` 开头，同一篇文章开头字节不同。idea 侧孪生（case-28 正文为空、仅凭 idea 生成）更无从比对。
2. **动态取样本会引入漂移**。任何"评测时从活库取近期文章"的方案，都会让作者每归档一篇新文章就改变注入样本，同一用例的分数随之变化，报告「上次总分/差值」一列沦为噪声——正是 24.2 刚治好的病。**量具的配置必须固定**。证据 2 直接否决了"用例声明 source_article_id"方案（既有每条新用例的维护负担与漏填即静默开洞，又不解决漂移）。

**用例池与样本池同源的结构性约束**：28 条用例中 26 条是「经典文学解读」（红楼梦人物书评），而作者库里的红楼篇几乎都已是用例来源（香菱→case-13/14/27/28、鸳鸯→case-25/26、晴雯→case-09/10）。因此必须留出「只做样本、永不做用例」的文章。

**实施**：
1. `evals/style_samples/` 存版本控制的固定样本，评测只读此目录，不碰活库。选定两篇（已验证与全部 28 条用例的最长逐字重合为 **0 字**）：《时间扑面而来，我们都在葬花》《清白的人，为什么总被当成异类》——均为情感文学，与红楼用例题材同源（拿技术文当样本会教出错的腔调），且不是任何用例的来源。**这两篇从此不得再做评测用例。**
2. `EvalStyleSampleLoader` 按文件名升序读取 `.md`（顺序固定即结果可复现）；`EvalPipelineFacade` 新增 `styleSamples` 注入 `style.sample_texts`，对齐 App `resolveStyle()` 的行为。
3. 报告头新增「风格样本」行，写明本轮注入了哪几篇（无样本时显式写「无（零样本配置）」）。样本换了分数即不可比，而同源判定无法可靠自动化，只能用可见性让人复核。
4. **防泄漏守卫** `EvalStyleSampleLeakTests`：每条用例 × 每份样本，检测任意位置长度 ≥60 的公共子串（去空白后），命中即失败并点名样本与用例。用长公共子串而非前缀指纹，正是为绕开证据 1 的 frontmatter 问题。样本目录为空时 skip。
5. 测试 2 项 + 报告断言 2 项；全量 190 项通过。守卫的**可失败性已实证**：临时把 case-27 正文投放为样本，测试如期失败并输出"风格样本「99-LEAK-TEST-TEMP」与用例 case-27 有 60 字以上的逐字重合"，随后清理。

**代价（必须周知）**：被测配置已改变，**历史基线（deep≈86.5、agent≈85.7、以及 24.2 之前的全部分数）作废**，需重跑一轮建立新基线。这是修复本项的固有成本，任何方案都躲不掉。

**残留边界**：守卫只能捕获逐字重合。若将来新增的用例与某篇样本**题材相同但文字不同**（例如为《葬花》另写一篇书评用例），守卫不会报警，仍需作者在评审用例时人工把关——报告头的「风格样本」行就是为这一步留的复核锚点。

### 24.5 评测与 App 的 prompt 路径合流（已实施，2026-07-25）

**背景**：24.2 定位到的根因是"App 走库内 seed 模板、内联 prompt 从未执行"，但当时只把锚点共用做到 `writingReview` 一个 key。继续审查发现同一条断链还有另一半，而且方向相反：**评测跑的是 App 从不执行的内联 prompt**。

**证据**：`EvalPipelineFacade.runDirect` 对成稿传 `template: nil`（内联 draft prompt），而 App 的"大纲成稿"传 `promptTemplate(for: .draft)`（`WorkshopStore.swift:799`），两份文案的写作要求不同——内联有"开头能让读者读下去""技术概念必须解释清楚定义、价值、例子和边界"，库内 seed 只有一行"严格沿大纲展开；段落自然；保留个人表达；结尾有回味"。agentic 更彻底：facade 构造 `WritingAgentSessionInput` 时五个模板参数一个都没传，全部默认 nil，而 App 侧五个都传（`WorkshopStore+AgentSession.swift:67-71`）。结论：**direct 与 agentic 的分数不代表 App 对应功能的质量**，L2 放行门当初判的那个 agentic 用的决策 prompt 也不是 App 会跑的那份。agent/deep 的成稿走 `AgentDraftCoordinator`（无模板参数），两侧本就一致，不受影响。

**实施**：
1. `EvalPipelineFacade` 新增 `promptTemplate(_:)`，从活库读模板；direct 的成稿、deep 生成闭环里的诊断、agentic 的决策/诊断/润色/成稿/大纲一律改用库内模板，与 App 同源。
2. **量具与被测物的分界写进代码注释并加测试守住**：生成路径跟 App 走库内模板；最终评分仍用写死在代码里的 `scoringTemplate`（22.4/24.4 的锚点版）。作者手改写作诊断模板不会动摇评分尺子。
3. 报告头新增「Prompt 模板」行，逐 key 标注"默认/作者自定义/缺失→内联"。prompt 换了分数就不可比，与 24.4 的「风格样本」行同一个道理：用可见性代替自动判定。
4. 测试 4 项 + 报告断言 1 项；全量 195 项通过。**可失败性已实证**：临时把 direct 的模板参数改回 nil、agentic 的决策模板改回 nil，两项守卫如期失败（"direct 管线应当发出库里的成稿模板"、"agentic 会话的决策调用应当用库里的模板"，另含"不应再发内联 prompt"一条），随后还原。

**代价**：被测配置再次改变，**24.4 之后跑出的那轮数据同样不能与本次之后的结果混作趋势**。建议与 24.4 的新基线一并重跑，一次付清。

**残留边界**：
- 内联 prompt 仍然留在 `NativePrompts` 里（模板缺失时的兜底），代码中两份文案并存的事实不变；只是两条路径现在都指向库内模板，不会再各跑各的。
- 评测现在会跟着作者手改的模板走。这是刻意的——评测就该量 App 实际在跑的东西——但代价是作者改一次 prompt、分数就换一次尺子，只能靠报告头的「Prompt 模板」行提醒复核。

### 24.6 风格样本三处缺陷：静默拼接、草稿当范本、跨体裁乱取（已实施，2026-07-25）

**背景**：24.3/24.4 把样本"有没有"修好之后，继续查"注入的是什么"，在作者活库里拿到三处证据。

**证据（作者活库实测）**：
1. **样本被静默拼接**。`NativePrompts.truncate(chunkSize: 800)` 对超 2400 字的文本取头 800 + 尾 800，中间只放一个空行——没有任何标注。模型读到的是一篇结构断裂却看不出断口的"作者范文"。`id=11`《鸳鸯》2615 字已经过线。这正是 24.3 给素材修掉的毛病，当时漏了样本这一路。
2. **草稿被当风格范本**。24.3 只给"精确匹配落空"的兜底分支加了 `finishedOnly`，精确命中的主路径照旧含草稿。实测 `genre='情感文学'` 命中的前两篇是 `id=14`（草稿）和 `id=13`（草稿，还是一篇 AI 主题文挂在情感文学名下）。
3. **兜底跨体裁取到技术文**。作者方向"经典文学解读"精确匹配 0 篇，必走兜底；实测兜底两篇是《叹香菱》(文学原著) + **《哀牢山地理志》(技术分享)**。而 24.4 为评测选样本时刚判过"拿技术文当样本会教出错的腔调"——同一件事两条相反的标准，App 侧走的还是更差的那条。

**根因**：`articles.genre` 存的是保存时的**写作方向原文**，作者库里已碎成六值（情感文学 5、空 4、文学原著 2、技术分享 2、科研技术 1、广告创意 1）。精确相等太脆（换个说法就归零），"落空就跨体裁乱取"又太糙。

**实施**：
1. `truncate` 的省略处显式写明"（此处省略中间 N 字）"，与 24.3 的素材处理对齐。头尾长度不变，只补标注。
2. 新增 `StyleSampleSelector`（Core 纯逻辑）：**体裁串共享 ≥2 个连续汉字即同族**——"经典文学解读" ↔ "文学原著" ↔ "情感文学" 靠"文学"连起来，"技术分享" ↔ "科研技术" 靠"技术"连起来，两族互不相认；1 个字不算，否则"文学原著"会连上"原创广告"。挑选**只取最高一档不混档**：精确 > 同族 > 跨体裁（跨体裁保守取 2 篇）。
3. `resolveStyle` 改为一次取近期**完成稿**作候选、体裁优先级交给 selector 判；只有一篇完成稿都没有（新库/全是草稿）时才退到草稿——范本不理想，但好过模型一篇都没见过作者的文字。选择逻辑外迁到 Core，`WorkshopStore.swift` 只增 14 行（1995 行，棘轮 2050 内）。
4. 测试 6 项（4 项 selector 规则 + 1 项 Store 级行为 + 1 项省略标注），全量 201 项通过。**可失败性已实证**：临时还原旧实现，三条断言如期失败（"省略处必须写明丢了多少字"、"只有《晴雯钻被窝》够格"、"技术文会教出错的腔调"），随后还原。

**实测效果**：方向"经典文学解读"下，注入样本从「香菱(文学原著) + 哀牢山地理志(技术分享)」变为「香菱、鸳鸯、晴雯」三篇同族文学文；`genre='情感文学'` 路径不再把两篇草稿当范本。
...
### 24.8 编辑量记录结构性失真：基准选错 + 重复计数（已实施，2026-07-25）

**背景**：作者首次把《叹香菱》走完发布流程，`edit_records` 终于从 0 变成有数据——但 7 条记录**全是 `0 增 0 删、ratio 0.0、zero`**，零改率 100%。北极星第一次有读数，读数却是假的。

**根因一：基准选错**。`latestConfirmedDraftVersion` 取最近一条 confirmed 版本，而"覆盖保存自动记版本"（保存可回退功能）写入的 `action = '保存文章'` 版本同样是 confirmed，内容就是作者当下的正文。于是比较的是"刚保存的那版 vs 刚保存的那版"，编辑比例**结构上恒为 0**。作者这篇的版本序列是决定性证据：

```
67  定点改写·语言腔调  2184   ← 最后一次模型产出，这才该是基准
68  保存文章          2184
69  保存文章          2185
70  保存文章          2187
71  保存文章          2186   ← 实际发布的内容
```

**根因二：重复计数**。发布记录挂在"状态是已发布"上而非"翻到已发布"上，已发布状态下每点一次「更新状态」或「保存」都追加一条。作者 7 次操作 → 7 条记录，条条零改动，把零改率钉死在 100%。

**实施**：
1. `latestConfirmedDraftVersion` → `latestModelDraftVersion`，排除 `action = DraftVersionAction.manualSave`（库里 14 种 action 只有「保存文章」是手动保存，其余都是模型产出）。常量提到 `WorkshopModels`，写库查库共用一处。
2. 排序补 `id DESC`：`created_at` 只到秒，模型动作后紧跟一次保存完全可能同秒，并列时"最近一版"不能随实现摇摆。
3. 记录时机改为**跃迁**：`updateSelectedArticleStatus` 与 `saveArticle` 都只在前一状态非「已发布」时记一条。归档后再发布算新的一次跃迁，仍会记。
4. 测试 2 项，全量 203 项通过。**可失败性已实证**：去掉 action 过滤，基准断言如期变成"保存文章"且编辑量为 0；去掉跃迁判断，重复发布记出 3 条。

**遗留**：作者本机那 7 条零改动记录仍在库里，会永久拉高零改率。是否清理待作者裁决——代码修复不追溯历史数据。

### 24.9 评测量具三处漏与调用韧性（已实施，2026-07-25）

**背景**：24.4/24.5 两次改被测配置后欠一轮新基线，作者于 2026-07-25 15:24 起跑全量。审查这一轮的运行状态时发现量具还有三处漏，且都直接落在"分数能不能读"上。

**根因一：失败样本照旧被打分，一路进报告、进差值、进放行门。** `OfflineBreaker.isInstantNetworkFailure` 要求 `elapsedMS < 5000`（设计时针对"离线错误 0-4ms 返回"），而**超时类失败耗时几百秒，熔断器照不到**，于是走完 `EvalPipelineFacade.score()` 拿到分数入库。三条决定性证据：

| 证据 | 数据 |
| --- | --- |
| 全库 `success = 0` 却有分数的行 | 68 行（其中耗时 ≥5s、熔断器盲区的 32 行） |
| 正在跑的这轮基线 | case-03/deep 失败 303s 记 83 分，case-05/deep 失败 487s 记 92 分 |
| 7-24 那轮报告的差值列 | −18/−27/−28 三个"退步"全部是拿失败样本当上次总分算出来的 |

分数从哪来的也已查清：生成失败时正文是 `NativeFallbacks` 的兜底稿；评分调用失败时 `NativeFallbacks.writingReview` 会按段落数与标题有无算出一个本地启发式分数（68 起步加减）——库里那些 58/66 就是它。两种情况都与锚点无关，却和真分数同列展示。后果最重的一环是 `ReleaseGate`：它是 L2 转默认的唯一裁决依据，却把失败样本混进平均值。

**根因二：网络类失败零重试。** `AIWorkflowRunner` 只对解析错误重试一次，`isParsingError` 注释明写网络/HTTP/取消不在此列。真实失败率不低（7-24 那轮 6 格里 5 格失败）。同一条路径 App 侧也在跑：作者点「大纲成稿」等最长 240 秒，一次超时就得到本地模拟稿，而 `finalizeGeneratedDraft` 在 `usedFallback` 时照样 `applyDraft` 并落一条 draft_version。

**根因三：评测不能断点续跑。** 全量 112 格约 8 小时（本轮 29 格已烧 128 分钟模型时间），行是增量落盘的，但没有任何续跑入口——中断就得从 case-01 重烧。

**实施**：
1. **失败样本不产出分数**（`EvalPipelineFacade`）：生成失败时直接不发评分调用（省一次模型调用，也从结构上保证无效样本没有分数可被误用），验证信号写"未评分：生成失败，本样本不计入分数与放行门"；评分调用自身失败时 `overallScore` 与 issues 一律留空，不让本地启发式分数冒充 rubric 分数。
2. **消费端一致过滤**：`previousRunScores` 只取成功样本（差值列不再凭空生成）；`ReleaseGate` 的分数平均只算成功样本，并在报告里写出"有效 n/m"，某条管线一格有效样本都没有时输出"有效样本不足，本轮不判定"。**调用次数与熔断率仍按全部样本算**——成本和出岔子的概率是管线的真实属性，剔掉失败样本反而会把熔断率这一项本身抹平。
3. **报告头点名无效样本**：`- 无效样本：N/M 格不计分、不进差值与放行门 —— case-03|deep（错误）`，全部成功时写"无（全部成功）"；表格里失败行的质量列（总分/问题数）一律画横线——"0 高危"会被读成质量好，而它其实只是没打分。成本列照旧出数。
4. **网络类失败退避重试**（`AIWorkflowRunner`，5 秒 / 20 秒两次）：超时、连接中断、TLS 握手失败、DNS 失败可退避重试；HTTP 错误（含 5xx/429）保持 PRD 22.3.3-A 的契约不变，仍不重试。重试痕迹写进 `inputSummary`/`error`（"网络重试 2 次后成功/仍失败"），`ai_calls` 与评测报告里看得见。间隔可注入，测试传 0。
5. **`--resume` 断点续跑**（`EvalResultsStore.latestRun/completedPairs/outcomes` + `main.swift`）：沿用最近一次 run 的 run_id 与时间戳，跳过**已成功**的格子（失败的格子重跑），报告补成整轮而不是留下两份各跑一半的；同一格重跑过则后写入的行胜出；新行记当前 git describe，报告头写成 `旧 → 新（续跑）`。
6. 测试 10 项，全量 213 项通过。**可失败性已实证**：把五处修复的行为逐一还原后重跑，9 项守卫如期失败共 22 条断言，且未波及任何既有测试。其中放行门那条最说明问题——失败样本计入后 deep 均分被拉到 69.00，agentic 75.00 凭一次网络抖动"通过放行门"。

**代价与边界**：
- 重试期间界面只显示原动作进行中，看不出"正在第 2 次尝试"；最坏情况一次调用要等 240+5+240+20+240 秒（动作可取消）。把重试进度接到 statusText 需要给 runner 加进度回调，本次未做。
- 兜底稿仍会被 `applyDraft` 写成待复核版本，若被作者确认，它仍可能当上 24.8 编辑比例的基准（`draft_versions` 没有记录"这一版是兜底产出"的列，要加需动 schema）。重试把这件事的发生概率压低了，但没有根除。
- **历史数据不追溯**：库里那 68 行"失败却有分数"仍在，读取端已按 `success = 1` 过滤，但历史报告里的数字仍是错的（尤其 7-24 那轮的差值列）。
- 正在跑的那轮基线是旧二进制产出的：剩余格子仍会出现"失败带分数"的行。建议跑完（或中断后）用新构建 `--resume` 把失败格子重跑替换——报告会按同一 run_id 重新生成，届时无效样本行会自动点名。

### 24.10 问题清单跨轮对比 + App 侧样本可见性（已实施，2026-07-25）

**背景**：24.2 的裁决是"趋势改用问题清单而非分数来读"，24.6 的残留边界写的是"注入了哪几篇样本 App 界面上仍然看不见"。两条都是已记录在案却没有工具支撑的结论——本节把它们补齐。

#### P1-4 问题清单跨轮对比（评测报告）

**证据**：`previousRunScores` 只取 `overall_score`，报告只有「上次总分/差值」一列；`PipelineOutcome` 里 issues 只剩高/中/低三个计数，**维度根本没落库**。于是 24.2 那条裁决在工具上无从执行：读报告的人能看到的仍然只有分数，而分数正是被判定为不可靠的那个量（三连诊断 83/73/72，同一份清单下抖动仅 1 分，10 分极差全来自档界跳变）。

**实施**：
1. `EvalPipelineOutcome`/`PipelineOutcome` 新增 `issueDimensions`，形如 `人称视角(中)`。**严重度必须带上**：24.2 第二例的主因正是同一条问题被重新定级（「语言腔调·中」→「文本引用与推断的边界·高」），只比维度名会把定级摇摆读成"问题没变"。
2. `eval_results` 新增 `issue_dimensions` 列（沿用 `addColumnIfMissing` 迁移；历史行读回是空清单，维度级趋势从本次之后的运行才有）。
3. `previousRunScores` → `previousRunSamples`：返回分数 + 三档计数 + 问题维度，仍只取成功样本（24.9）。
4. 新增纯函数 `IssueTrend.describe`，报告按用例逐管线出一行：`- deep 问题清单：人称视角(中)、意象与细节(中)、留白与节奏(中)；中 2→3；新增 留白与节奏(中)`。清单逐条相同时明说「与上轮逐条相同」——这正是 24.2 第二例要看的形状（问题稳定、分数跳 10 分）。失败样本不出这一行。
5. **App 侧无需改动**：写作教练已有 `resolved_from_last`「已解决（较上次诊断）」行，Inspector 已有「文学写作能力雷达」（历史诊断维度频次的前后半段对比）。裁决所需的问题清单视角在 App 内本就存在，缺的只有评测这一侧。

#### P1-5 App 显示本次注入了哪几篇样本

**证据**：评测报告 24.4 起就有「风格样本」行，App 侧 `resolveStyle()` 注入完样本不留任何痕迹。作者怀疑"写出来不像我"时只能查库——而 24.6 刚证明过样本可能取到草稿、甚至技术文。

**实施**：
1. `StyleSampleSelector` 新增 `Tier`（精确/同族/跨体裁/无）与 `selection(from:genreKey:)`，`select` 转调它；命中档位从此可上报。
2. 新增 `StyleSampleProvenance`（档位 + 篇名 + 是否草稿兜底），`resolveStyle()` 每次更新 `WorkshopStore.lastStyleSampleProvenance`，随 `PendingReviewMachine.deliver` 进 `PendingDraftReview`。
3. 待复核卡渲染一行：「本次风格样本：叹香菱、晴雯钻被窝（体裁精确匹配）」；无样本或草稿兜底时标橙色——`未注入样本`与`草稿兜底`都是需要作者知情的降级。
4. 测试 11 项（6 项 IssueTrend + 2 项报告/落库 + 1 项档位 + 2 项 Store 级出处），全量 224 项通过。**可失败性已实证**：两轮行为还原共让 7 项守卫失败 17 条断言（报告不出清单行、维度不落库、严重度不带档、档位一律报"精确"、出处不进待复核卡、草稿兜底不标记）。
...
### 24.13 评分示例值即锚点：分数是问题清单的查表结果（部分实施，2026-08-15）

**背景**：24.2 把"分数在 79/80 档界跳 10 分"裁决为"锚点的设计后果，不是失真"，并改口径为"趋势看问题清单不看分数"。本次对整条评分链路（few-shot 样本注入 → 五份 prompt 模板 → 评分 → 落库 → 放行门）做一次完整审计，发现档界跳变只是表象，底下还有一层更硬的东西。

**证据（run 2026-07-25，100 个有效样本）**：
...
按问题清单分组后，尺子实际只有四格：0 高 0 中 → 88.6；0 高 1–2 中 → 83.4；1 条高危 → 76.4；2 条及以上高危从未出现。单条高危 ≈ −8.9 分，单条中危 ≈ −5.0 分，第二条中危 ≈ −0.5 分。**"75 分"不是质量位置，是"评委列了 1 条高危"这个二值事件的落点。**

**根因（示例值即锚点）**：分档文案负责挡住 80 分线（"有高危问题不得进入 80 档"，这解释了 80–82 为什么是空的），但决定落在哪一格的是输出示例里的那个具体数字。历史提供了一次天然对照——首轮（`40e832c`）示例写 `"overall_score":72`，23 个有效样本里 **12 个正好 72**（52%）；`4e7a9d3` 改成 83 之后，众数当场搬到 83，此后三轮稳居第一（12/55、12/26、41/100）。即：24.2 加的五行锚点没有失效，但它管的是闸门，不是刻度；真正的刻度一直是示例里那个数。

**实施**：
1. 两处示例值改成占位符：量具 `EvalPipelineFacade.scoringTemplate`（原 83）与 App 写作教练种子 `writing_review`（原 72），均改为 `<按…锚点确定的整数>`，并在"请只输出 JSON"那行补明"尖括号是占位说明，overall_score 必须是不带引号的整数"。占位符刻意不用 `{{}}`——24.11 的第三条守卫会把渲染后残留的 `{{` 判红。
2. `WritingReviewResult.overall_score` 改走宽松解码（与 `TopicPayload.score` 同一处理）。这是前一条的必要配套：示例值不再是数字后，模型偶尔会把分数当字符串回成 `"83"`，而合成 Codable 在这里抛 `typeMismatch`，**整条诊断连同 issues 一起解码失败**——分数字段的一个类型抖动会赔掉整次调用。
3. 两处注释写死"不许填回具体数字"的理由（`scoringTemplate` 与 `scoreAnchorBlock` 各一份），并把 72→83 的对照数据写进注释——这类改动最容易被后人"顺手补个示例值"还原。
4. 测试 2 项：`testWritingReviewResultDecodesStringifiedScoreWithoutLosingTheRest`（`" 83 "` 应解出 83 且 issues 不受连累）、`testWritingReviewResultKeepsNilScoreWhenUnparseable`（占位符原样回传时留 nil 而不是抛错，评测侧靠 nil 触发重评）。全量 320 项通过（318 → +2），`script/verify.sh` 守卫与测试全绿。

**迁移**：无需手写迁移。`seedPromptTemplatesIfNeeded` 的刷新分支只更新 `is_default = 1` 且未手改过的行；07-25 报告头记录五份模板全为"默认"，作者本机下次启动自动生效。

**这一条构成 L2 重启条件里的"prompt 实质改动"，2026-07-25 基线作废。** 与 24.12 不同，这次改的是量具本身且落在每一次实际执行的评分调用上，跨轮分数不再可比。

**验证口径**：重跑评测，看 83 分的占比从 41% 掉到多少。掉下来说明此前那 41 个里有相当部分是锚定效应；不掉说明 83 确实是这批稿子的真实水平，那就要回头查分档文案。此项必须全量重跑才有意义，抽样看不出众数。

**本次审计发现但未实施的（按建议优先级）**：
1. **问题清单没有落盘**：`raw_json` 只存三个计数，`issue_dimensions` 列在当前结果库里根本不存在（迁移要等下次开库才跑）。已落盘 12 轮 283 行没有一条能回答"哪条问题把它按在了 75"——24.2 那条"趋势看问题清单不看分数"的裁决因此从未真正可执行。**正文存了**（`raw_json.content`），所以重判不必重新生成，只跑评委那一次调用即可。
2. **放行门第一项判不出来**：deep 85.17（n=24）对 agentic 84.44（n=25），差 0.73，差值标准误 1.40——0.52 个标准误，95% 置信区间 −2.02 ~ +3.48 跨 0；而这把尺子最小刻度是"一条中危 ≈ 5 分"。要在这个量级判高低需每管线约 174 例。`ReleaseGate.swift` 写的是不带不确定性的 `>=` 硬比较，正确读法是"判不出"而非"不通过"（记忆与既有报告里的"差 0.73 分，不通过"应一并改口径）。顺带：被这 0.73 分挡住的 agentic，高危率是四条管线里最低的（2/25）。
3. **`draftCritique` 拿不到风格样本**：few-shot 只注入 `writingBrief` / `sectionDraft` / `improveDraftFromReview` / `draft` / `polishDraft` 五处，而 `draftCritique`（自我批评与整合）是 agent 管线的**末步**且会重写全文（`AgentDraftCoordinator.swift:154`）。数据吻合：agent 均分四条最低（82.69）、高危率最高（5/26）；direct 只调 2 次却 84.56；deep 在其后补一刀带样本的 `improveDraftFromReview`，回到 85.17 第一。
4. **量具与共享纪律块未解耦**：`scoringTemplate` 有"改这份 = 作废历史基线"的注释保护，但渲染后还会拼 `agentDiscipline(mode: .review)`（`NativePrompts.swift:355`），这段不受保护。`16ec19d`（2026-07-10）给它加了"体裁适配"条款，明确要求解读类体裁"不要建议加入个人生活体验"——而用例库 27/28 正是经典文学解读。**基线实际断在 07-10，报告头捕捉不到这次改动。**
5. ~~**同类数字锚点还有两处未动**~~ **已补做（同日，作者追加）**：`topics` 种子 `"score":4`（1–5 推荐指数）与 `judgeDraftCandidates` 种子 `"score":86`（候选排序）一并改为占位符。两点值得记：
   - **`judgeDraftCandidates` 的 score 从来没定义过量程**，`86` 是唯一的暗示——直接删数字会把量程一起删掉，所以同时补了"score 用 0-100 表示这一版的整体可用度，不同候选的分数要拉开"。`topics` 那处不存在这个问题，模板正文早就写着"推荐指数 1-5"。
   - **`CandidateScore` 比诊断那处更脆**：`candidate_index` 与 `score` 都是非可选 `Int` 且嵌在 `rankings` 数组里，模型回一个 `"86"`，赔掉的不是一个字段而是整份候选排序。两个字段改为接受"整数"与"能解析成整数的字符串"，但**只放宽类型、不放宽字段缺失**——字段真没有时照旧抛错走 fallback，兜一个 0 会造出指向不存在候选的排序，比失败更难查。测试 2 项（`testCandidateJudgeResultDecodesStringifiedScores`、`testCandidateScoreStillFailsWhenScoreIsMissing`），全量 322 项通过。
   - 二者均不在评测计分路径上，**不影响 24.13 的基线判定**。
6. **`judgeDraftCandidates` 的 `best_candidate_index":1` 未动**：它不是分数锚点而是位置锚点——LLM 评委的位置偏好是已知效应，示例里写 `1` 可能把评委推向第一个候选。与本节其余各条机制不同（那些是"示例值决定取值"，这条是"示例值决定选择"），且验证方法也不同（要看历史 `best_candidate_index` 的分布是否偏 1，而不是看众数），单列待议。
7. ~~**用例库体裁单一**~~（见下，仍未动）：28 个用例里 27 个 direction 为"经典文学解读"，`evals/cases/README.md` 自己写的建议是"覆盖不同方向/体裁，避免评测结果被单一题材主导"。另：`EvalPipelineFacade.swift:175` 无条件前置两篇固定样本，绕过了 24.6 给 App 做的体裁同族匹配——唯一的例外用例 case-23（武侠风格练习）拿到的两篇范本是情感随笔。

### 24.14 配对重判：把"哪个维度拖分"与"改动有没有效果"变成可查的（已实施，2026-08-15）

**背景**：24.13 交付后追问三件事——(1) 现有记录里总分被哪个维度拖得最多、这些低分样本有什么共同特征；(2) 如果只能改一处改哪里、预期提升多少；(3) 把改动做成可复跑的改前/改后对照。查库后发现前两问的答案都卡在同一个地方：**数据不存在**。

**先说查库结果（不是直觉，是这两个库里能查到的全部）**：

*评测库*：`eval_results` 100 个有效样本，`issue_dimensions` 列至今不存在，`raw_json` 只有三个计数。**维度数据为零。**

*App 库*（`writing_reviews`，72 条，是唯一存了完整 issues 的地方）——三处混杂让它同样回答不了：

| 混杂 | 实测 |
| --- | --- |
| 锚点后样本量 | 15 条诊断，**全部来自 2 篇文章**（article_id 15 九次、16 六次，含 24.2 的三连诊断实验） |
| 维度词表在同一天换过 | 锚点前 10 个词（开头/结构/观点/素材…），锚点后 11 个（人称视角/意象与细节/留白与节奏/语言腔调…），**交集只有 4 个**，池化即混taxonomy |
| 高危样本 | 锚点后**只有 1 条**（锚点前 20 条，但分数与词表都不可比） |

*从锚点前 56 条里能捞出的唯一干净信号*：高危维度排第一的「正文」8 次、平均 58 分，逐条看下去**全是"正文为空"**——那是生成失败时 `NativeFallbacks` 的兜底诊断，不是质量维度（正是 24.9 从主路径拆掉、但历史记录里仍在的那个病）。剔掉它之后，**结构**才是真正的头号：出现在 37/56 份诊断里，带 5 条高危，带此维度的稿平均 72.4、不带的 79.4。

*锚点后唯一那条高危*（id 66，75 分）：维度「文本引用与推断的边界」，原文"曹雪芹的香菱，判词里写得清清楚楚……没有反转，没有公道"，判词是"将基于判词的推测当作确凿事实"。**这条与评测用例高度相关**（27/28 个用例是经典文学解读，正是最容易犯这个错的体裁），但 n=1，是假设不是结论。

**裁决**：现有记录**不足以**回答"哪个维度拖分最多"。任何基于上表给出的排序都是在描述两篇文章或一套废弃词表。因此本次不实施任何生成侧改动，先把这个问题变成可查的。

**实施（`--rejudge`，只重判不重生成）**：
1. `EvalPipelineFacade.ScoringVariant`：把评分模板的示例位变成**显式参数**（`anchored83` / `placeholder`），两臂共用同一份正文字符串。关键取舍是**把改动前的版本留在代码里而不是留在 git 历史里**——否则每次对照都要切分支重新构建，成本高到不会真去做。
2. `EvalPipelineFacade.rejudge(...)`：只发评分那一次调用，输入是 `eval_results.raw_json.content` 里的存量正文。两臂拿到的是**同一个字节序列**，属配对设计。
3. `RejudgeStore` / `rejudge_results` 新表：记录完整 issues（维度 + 严重度 + **原文片段** + 判词），这是 24.13 环 6 缺的东西。与 `eval_results` 分表——那张是基线，形状变了跨轮不可比；重判是可随时加变量的实验，不该污染基线。
4. `RejudgeReport`：配对差值 + 差值标准差/标准误/95% 区间 + 升降不变分布 + **83 分占比**（本次改动最直接的观察点）+ 两臂各自的维度频次表（按高危数排序）+ B 臂高危问题原文。
5. 测试 10 项，其中最要紧的是 `testTwoScoringArmsDifferOnlyInTheScoreExampleSlot`——**这是整套实验的地基**：谁改了其中一臂的措辞，A/B 就从单变量变成双变量，而这种漂移不会让任何别的测试变红。全量 332 项通过（322 → +10）。

**为什么配对是关键**：24.13 算过"deep 对 agentic 差 0.73 分要每管线 174 例才判得出"——那是**非配对**比较，组间方差里绝大部分来自"这些稿子本身有好有坏"（总标准差 4.64）。配对之后这一项被完全消掉，剩下的只有变量效应加评委噪声。报告会直接印出"检出 1 分效应需多少格"，用当轮实测的差值标准差算，不用拍脑袋。

**运行方式与成本**（作者本机终端跑，见 README）：

```bash
caffeinate -i .build/debug/CreativeWorkshopEval --rejudge latest --pipelines direct   # 试跑 50 格
caffeinate -i .build/debug/CreativeWorkshopEval --rejudge latest                      # 全量 200 格
```

来源默认取最近一轮（2026-07-25，100 格成功且正文非空 → 200 格重判，正文平均 2742 字）。`--resume` 可续跑，唯一索引保证重跑覆盖而非叠加。**生成侧零调用**——对比全量重跑一轮 112 格约 8 小时。

**这一条不改变被测对象，不影响 24.13 的基线判定**：重判读的是存量正文，不动生成路径，也不写 `eval_results`。

**残留**：
- 重判只覆盖评分侧变量。生成侧改动（如 `draftCritique` 补样本）仍需真的重跑生成，配对只能配到"同用例同管线"这一层，配不到"同正文"。
- `rejudge_results` 目前只按 `rejudge-<source_run_id>` 分批，同一来源换第三个变量会与前两臂落进同一批；要三臂对照需先扩批次命名。
- 维度归一只剥括号补语与首个分隔符，同义异名仍算两个维度。**2026-09-13 复议：终判不做受控词表。**
  真诊断（63 次，已排除兜底）归一后只剩 15 个维度，逐对检查下来能合的几乎没有：「文本引用关系」
  与「文本引用与推断的边界」确实同义，但各 1 次，合并后 2 次，改变不了任何读数；而「留白与节奏 4×」
  并进「节奏 22×」会抹掉留白这个独立的文学关切，「意象与细节 4×」与「素材 20×」是修辞与来源两回事，
  「语言腔调 10×」与「风格 6×」是语域与整体两回事——**误合的风险是实的，收益是零**。
  原残留举的例子「腔调」在真诊断里出现 0 次。若将来维度种类显著增多且出现真实的重复计数，再重开。

### 24.15 「按诊断改全文」的可靠性：L2 结案时剥离出来的独立问题（待办，2026-09-12 立项）

**为什么单独立项**：这条发现来自 L2 的会话轨迹，但责任不在 L2。结案 23.6 时若不把它剥出来，它会跟着 L2 一起被埋掉。

**证据**：真实会话 138（2026-07-24，香菱一文）以"连续两轮验证信号未变化"熔断，8 步轨迹是：

1. 写作诊断 → 报出高危问题「第四与第五部分之间出现『...章节。』的断裂，疑似缺漏或未完成的过渡」；
2. 按诊断修订 → 重写全文；
3. 再次诊断 → **同一处结构断裂仍在**（严重度从高降到中，问题描述变成「缺少『## 四』标题与必要过渡」）；
4. 于是调度器判断"改不动"，转去生成大纲——在一篇已成稿的文章上，这是明显的乱走，被熔断拦下。

**判读**：诊断准确定位了问题（两轮都指向同一处），执行动作没有解决它。也就是说这里失效的是**被调用的动作**（`improveDraftFromReview`），不是调用它的调度器。L2 的贡献恰恰是：连续两轮的对比把这个动作的不可靠**暴露**了出来——单次调用时作者只会看到"改了一版"，看不到"该修的没修"。

**为什么值得单独查**：这个动作在 L1 主线里也在用（「诊断复核」屏的"按诊断改全文"按钮），且是 deep 管线的组成部分。它的可靠性问题会同时影响默认路径，与 L2 是否转默认无关。

**下一步**（未启动）：用配对重判的思路验证——取若干条"诊断报出高危结构问题"的存量正文，跑一次 `improveDraftFromReview`，再诊断，统计**同一维度问题的消解率**。这比分数更直接：分数会因为别处变好而上升，掩盖掉"该修的没修"。

**不做的事**：在拿到消解率之前不改 prompt。24.13 的教训是改量具与改被测对象必须分开，这里同理。
